...
- **Node.js ≥ 22.5**（后端使用内置 `node:sqlite` 存广场数据；见 `server/package.json` 的 `engines`）
...
- `client/src`：React 界面；**读猫话**在请求生成时于主白卡片内（与选图/结果同宽）展示猫图轮播等待，不再使用全屏沉浸层。生成出结果后可用「**保存**」将 **猫图 + 心里话** 截成一张 PNG。可选：按引导将**同一张长图**与文案发到 **猫猫心里话广场**（`POST /api/plaza/posts`，须登录）；**逛广场、看图**为公开接口（`GET /api/plaza/posts`、`GET /api/plaza/files/…` 无需密码）。图存 `server/uploads/plaza/`，元数据在 `server/data/plaza.db`（部署时需持久化，见 `docs/DEPLOY_ALIYUN_LIGHT.md`）。评论/点赞为后续扩展位，当前未实现。
